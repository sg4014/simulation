...
void updatePositionsToUndoIntersection(Body& b1, Body& b2) {
    const auto posDiff = b2.getPosition() - b1.getPosition();
    if (posDiff.lengthSquared() == 0) {
        b2.move({b2.getRadius(), 0});
        return;
    }
    ...
    const auto distBetweenCenters = posDiff.length();
    const auto radiusSum = b1.getRadius() + b2.getRadius();
    ...
    const auto normalizedAxis = posDiff.normalized();
